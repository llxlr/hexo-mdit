...
function logic_js(str, data) {
    ...
    // Build uglify-js v3 options, stripping custom keys (enable/logger/stamp/exclude/fromString)
    var uglifyOpts = {};
    if (options.mangle) {
        uglifyOpts.mangle = typeof options.mangle === 'object'
            ? {
                toplevel: options.mangle.toplevel || false,
                reserved: options.mangle.except || options.mangle.reserved || []
              }
            : options.mangle;
    }
    if (options.compress && typeof options.compress === 'object' && Object.keys(options.compress).length) {
        uglifyOpts.compress = options.compress;
    }
    if (options.output && typeof options.output === 'object' && Object.keys(options.output).length) {
        uglifyOpts.output = options.output;
    }

    var result = UglifyJS.minify(str, uglifyOpts);
    if (result.error) {
        if (options.logger) {
            var logErr = hexo.log || console;
            logErr.log('minify js error: %s [%s]', path, result.error.message);
        }
        return str;
    }
