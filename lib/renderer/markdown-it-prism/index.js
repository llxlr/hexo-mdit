...
// diff-highlight 插件依赖 diff 语言定义，必须预加载
loadLanguages(['diff']);
...
module.exports = function (md, options) {
    ...
    Prism.hooks.add('wrap', function (env) {
        if (env.type == 'comment') {
            env.content = pangu.spacingText(env.content)
        }
    });
    ...
    md.renderer.rules.fence = (tokens, idx, options, env, self) => {
        ...
        let text = token.content.trim()
        let code = null

        let [langToUse, langShow, prismLang] = selectLanguage(config, lang);
        const ext = ` data-ext="${(langToUse || lang) == 'raw' ? '' : (langToUse || lang)}"`;
        ...
        const dataAttrs = ext + (runnable ? ' data-runnable="true"' : '');
        log.debug(`[prism-fence] lang="${lang}" langToUse="${langToUse}" caption="${caption}" mark=${JSON.stringify(mark)} command=${JSON.stringify(command)} firstLine=${firstLine} file="${file || ''}" from=${from} to=${to} raw="${info}"`);

        if (file) {
            const fp = path.join(config.source_dir, config.code_dir, file);
            text = fs.existsSync(fp) ? `${fs.readFileSync(fp)}`: '';
        }

        if (prismLang) {
            code = Prism.highlight(unescapeSwigTag(text), prismLang, langToUse);
        } else if (lang == 'raw') {
            code = escapeHTML(pangu.spacingText(unescapeSwigTag(text)));
            langShow = null;
        }
        ...
        if (file) {
            ...
            if (text.length !== 0) {
                ...
                if (from) {
                    startLine = (Number(from) <= lines?.length) ? Number(from) - 1 : 0;
                }
                if (to) {
                    endLine = (Number(to) >= startLine) ? Number(to) : 0;
                }

                for (let i = startLine, len = Math.min(endLine, lines?.length); i < len; i++) {
                    let line = lines[i];
                    let lineno = Number(firstLine) + i - startLine;

                    content += `<tr${(mark && mark.includes(lineno)) ? ' class="marked"' : ''}>` +
                               `<td data-num="${lineno}"></td>` +
                               `<td><pre>${line}</pre></td></tr>`;
                }
            } else {
                caption = '<span>Error: File Not Found</span>';
                content = `<tr><td data-num="1"></td><td><pre>File "${file}" Not Found.</pre></td></tr>`;
            }

            let result = `<figure class="highlight${langToUse ? ` ${langToUse}` : ''}">`;

            result += `<figcaption data-lang="${langShow ? langShow:''}"${dataAttrs}>${caption}</figcaption>`;
        ...
        if (lang == 'info') {
            return `<pre class="info"><code>${escapeHTML(pangu.spacingText(unescapeSwigTag(text)))}</code></pre>`;
        } else {
...
function loadPrismLang(lang) {
    ...
    if (langObject === undefined) {
        ...
        // loadLanguages resolves aliases to canonical names (e.g. f90→fortran, js→javascript),
        // but Prism.languages only stores the canonical name. Resolve the alias manually.
        if (langObject === undefined) {
            for (const name of Object.keys(components.languages)) {
                if (name === 'meta') continue;
                const comp = components.languages[name];
                if (typeof comp === 'object' && comp.alias) {
                    const aliases = Array.isArray(comp.alias) ? comp.alias : [comp.alias];
                    if (aliases.includes(lang)) {
                        langObject = Prism.languages[name];
                        break;
                    }
                }
            }
...
function getOptions(info) {
    ...
    const rRunnable = /\s*runnable:(true|false)/i;
    const rCommand = /\s*command:\((\S[\S\s]*)\)/i;
    const rSubCommand = /"+(\S[\S\s]*)"(:([0-9,-]+))?/i;
    const rFile = /\s*file:(\S[\S\s]*)/i;
    ...
    let from = 0;
    if (rFrom.test(info)) {
        info = info.replace(rFrom, (match, _from) => {
            from = Number(_from);
            return '';
        })
    };
    ...
    let to = 0;
    if (rTo.test(info)) {
        info = info.replace(rTo, (match, _to) => {
            to = Number(_to);
            return '';
        })
    };
    ...
    let runnable = false;
    if (rRunnable.test(info)) {
        info = info.replace(rRunnable, (match, _runnable) => {
            runnable = _runnable.toLowerCase() === 'false' ? false : true;
            return '';
        });
    }
    ...
    let file = null;
    if (rFile.test(info)) {
        info = info.replace(rFile, (match, _file) => {
            file = _file
            return '';
        })
    };
