...
module.exports = function(md, opts) {
  ...
  function renderAnchorLink(anchor, tokens, idx) {
    var attrs = [];
    if (opts.permalinkClass != null) {
      attrs.push(['class', opts.permalinkClass]);
    }
    ...
    var symbolTokens = [];
    if (opts.permalinkSymbolClassName) {
      symbolTokens.push(
        Object.assign(new Token('span_open', 'span', 1), {
          attrs: [['class', opts.permalinkSymbolClassName]]
        }),
    ...
    var method = opts.permalinkSide === 'left' ? 'unshift' : 'push';
  ...
  md.block.ruler.before('paragraph', 'toc_block', function(state, startLine, endLine, silent) {
    ...
    if (!TOC_RE.test(state.src.slice(pos, max))) return false;
  ...
  md.inline.ruler.after('emphasis', 'toc', function(state, silent) {
    ...
    if (!TOC_RE.test(state.src.slice(state.pos))) return false;
